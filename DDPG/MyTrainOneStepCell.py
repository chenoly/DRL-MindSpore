...
class ActorWithLossCell(nn.Cell):
    def __init__(self, critic, actor, loss_fn):
        super().__init__()
        self._critic = critic
        self._actor = actor
        self._loss_fn = loss_fn

    def construct(self, state, labels):
        q = self._critic(state, self._actor(state))
        return self._loss_fn(q, labels)
...
class ActorGradWrap(nn.Cell):
    ...
    def construct(self, state, labels):
        ...
        return self.grad(self.loss_network, weights)(state, labels)
